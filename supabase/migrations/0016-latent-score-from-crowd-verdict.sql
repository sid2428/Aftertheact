-- Fix: ContestantEpisodeAppearance.latent_score was never derived — it was only
-- ever the admin "Self Score" form field (default 0), so the scoreboard /
-- episode / homepage rankings (all ordered by latent_score) showed dummy values.
-- Latent Score is defined as the pure crowd verdict: the final trust-weighted
-- peoples_verdict_weighted, captured at reveal so it never appears on the public
-- scoreboard while an episode is still LIVE.
--
-- Recreate the reveal RPC (CREATE OR REPLACE, same as 0013→0015) with one added
-- step that copies peoples_verdict_weighted → latent_score for the episode.
...
BEGIN
    ...
    -- 4b. Latent Score = pure crowd verdict. Set at reveal from the final
    -- trust-weighted peoples_verdict_weighted so the headline ranking metric is
    -- real instead of the admin-entered placeholder. WHERE clause satisfies
    -- pg-safeupdate.
    UPDATE "ContestantEpisodeAppearance"
    SET latent_score = peoples_verdict_weighted
...
-- Backfill: already-revealed episodes scored before this fix kept their dummy
-- latent_score. Re-derive from the crowd verdict for any revealed episode.
UPDATE "ContestantEpisodeAppearance" cea
SET latent_score = peoples_verdict_weighted
FROM "Episode" e
WHERE cea.episode_id = e.id
  AND e.is_revelation_triggered = true
