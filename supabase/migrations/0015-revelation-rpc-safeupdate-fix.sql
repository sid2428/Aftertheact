-- Fix: "Failed to reveal episode: UPDATE requires a WHERE clause".
-- The DB has pg-safeupdate enabled, which rejects the global oracle_score
-- recompute in score_episode_predictions() because it had no WHERE clause.
-- Re-create the function with `WHERE u.id IS NOT NULL` (matches all users).
