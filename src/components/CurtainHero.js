...
function Face({ member, delay, tilt }) {
  return (
    <motion.div
    ...
    >
      {/* Tooltip on hover */}
      <div className="pointer-events-none absolute -top-2 left-1/2 z-30 -translate-x-1/2 -translate-y-full whitespace-nowrap rounded-sm border border-latent-gold/30 bg-[#111111] px-3 py-1.5 text-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
      ...
      <div className="relative w-24 h-24 lg:w-32 lg:h-32 overflow-hidden rounded-md border-2 border-white/10 bg-[#111111] shadow-[0_8px_32px_rgba(0,0,0,0.6)] transition-colors duration-300 group-hover:border-latent-gold group-hover:shadow-[0_0_30px_rgba(212,175,55,0.5)] flex items-center justify-center">
        {member.image ? (
      ...
      <div className="relative z-10 flex flex-col items-center justify-center gap-8 px-6">
        <div className="flex items-center justify-center gap-6 lg:gap-16">
          <div className="hidden md:flex items-center gap-6 lg:gap-10">
            {left.map((m, i) => (
              <Face key={`l${i}`} member={m} delay={1.6 + i * 0.15} tilt={i % 2 === 0 ? -3 : 3} />
            ))}
          </div>
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-10">
            {right.map((m, i) => (
              <Face key={`r${i}`} member={m} delay={1.6 + i * 0.15} tilt={i % 2 === 0 ? 3 : -3} />
            ))}
          </div>
        </div>

        {/* Tagline removed to prevent overlap with CTAs on small screens */}

        {/* Mobile judge strip — all judges in a scrollable row */}
        {members.length > 0 && (
          <div className="md:hidden flex gap-4 overflow-x-auto p-2 max-w-full">
            {members.map((m, i) => (
