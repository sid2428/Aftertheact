...
// Upload filenames are UUID-based (saveUploadedImage → randomUUID().ext), so
// each URL is content-addressed. Replacing a photo produces a new UUID URL;
// the old one becomes orphaned. Cache headers are `immutable` — no need for
// manual ?v= cache-busting.
...
export async function setPanelMembers(members) {
  await redis.set(KEY, members.map((m) => withId(m)));
}
